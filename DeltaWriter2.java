			...
			for(int i = 0; i < 3; i++)
		    {
		        ...
		        else if(delta_type == 2)
		        {
		            ...
		            if(i == 0)
		            {
		            	...
		            	ArrayList pixel_histogram_list = StringMapper.getHistogram(pixel);
		            	int pixel_min                  = (int)pixel_histogram_list.get(0);
                        int [] pixel_histogram         = (int[])pixel_histogram_list.get(1);
					    int value_range                = (int)pixel_histogram_list.get(2);
						int [] pixel_table             = StringMapper.getRankTable(pixel_histogram);
						int n                          = pixel_histogram.length;
						
						ArrayList frequency_list = new ArrayList();
					    for(int k = 0; k < n; k++)
					        frequency_list.add(pixel_histogram[k]);
					    ...
					    long [] huffman_code     = CodeMapper.getCanonicalCode(huffman_length);
					    ...
					    System.out.println("Huffman compressed pixel bit length is " + huffman_compressed_length);
					    System.out.println("Unary packed pixel bit length is " + unary_compressed_length); 
					    
					    // Since the first value is not a code, we use the alternative method.
					    ArrayList pixel_string_list = StringMapper.getStringList2(pixel);
						pixel_min                   = (int)pixel_string_list.get(0);
						int pixel_bitlength         = (int)pixel_string_list.get(1);
						pixel_table                 = (int [])pixel_string_list.get(2);
					    ...
					    for(int k = 0; k < n; k++)
					        frequency_list.add(delta_string_histogram[k]);
					    Collections.sort(frequency_list, Comparator.reverseOrder());
					    frequency = new int[n];
					    for(int k = 0; k < n; k++)
					    	frequency[k] = (int)frequency_list.get(k);
		        ...
		        else if(delta_type == 5)
		        {
				    ...
				    int n                        = map_histogram.length;
				
				    ArrayList frequency_list = new ArrayList();
			        for(int k = 0; k < n; k++)
			    	    frequency_list.add(map_histogram[k]);
			        Collections.sort(frequency_list, Comparator.reverseOrder());
			        int [] frequency = new int[n];
			        for(int k = 0; k < n; k++)
			    	    frequency[k] = (int)frequency_list.get(k);
			        ...
			        int [] unary_length = CodeMapper.getUnaryLength(frequency.length);
			        int [] unary_cost   = CodeMapper.getUnaryLength(frequency.length);
			        ...
			        System.out.println("Huffman compressed bit length is " + huffman_compressed_length);
			        System.out.println("Unary compressed bit length is " + unary_compressed_length);
			        ...
			        System.out.println("Zipped byte length of map is " + zipped_length);
			        System.out.println("Zipped length less huffman compressed length is " + (zipped_length - (huffman_compressed_length / 8)));
