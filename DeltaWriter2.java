...
public class DeltaWriter2
{
	...
	public static void main(String[] args)
	{
		...
		//DeltaWriter2 writer = new DeltaWriter2(filename);
	}
	...
	class ApplyHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			for(int i = 0; i < 3; i++)
			{
		        ...
		        byte [] compression_string;
		        if(zero_one_ratio > .5)
		        	compression_string = StringMapper.compressZeroStrings3(string, channel_length[j]);
		        else
		        	compression_string = StringMapper.compressOneStrings3(string, channel_length[j]);
			...
			for(int i = 0; i < 3; i++)
		    {
			    ...
			    if(channel_length[j] != channel_compressed_length[j])
			    {
			    	...
			    	*/
			    	...
			    	byte _iterations = StringMapper.getIterations(string);
			    	byte string_type = StringMapper.getType(string);
			    	
			    	System.out.println("Input string type is " + string_type + ", length is " + bitlength + ", iterations is " + _iterations);
			    	
			    	byte [] decompressed_string;
			    	if(channel_string_type[i] == 0)
			    		decompressed_string = StringMapper.decompressZeroStrings3(string);
			    	else
			    		decompressed_string = StringMapper.decompressOneStrings3(string);
			    	
			    	int decompressed_length = StringMapper.getBitlength(decompressed_string);
			    
			    	bitlength  = StringMapper.getBitlength(decompressed_string);
			    	_iterations = StringMapper.getIterations(decompressed_string);
			    	string_type = StringMapper.getType(decompressed_string);
			    	System.out.println("Output string type is " + string_type + ", length is " + bitlength + ", iterations is " + _iterations);
			    	System.out.println();
			    	
			    	if(decompressed_length != channel_length[j])
			    	{
			    		System.out.println("Decompressed length " + decompressed_length + " different from original length " + channel_length[j]);
