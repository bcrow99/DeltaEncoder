...
public class TableTester
{
	...
	class ApplyHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
		    ...
		    System.out.println("The low value is  " + green_min);
		    System.out.println("The high value is " + green_max);
		    ...
		    /*
		    green_shrink = DeltaMapper.shrink4(green_double, xdim, ydim, error_double);
		    green_expand = DeltaMapper.expand4(green_shrink, xdim / 2, ydim /2);
		    ...
		    System.out.println("Biased error shrinking with error is " + biased_error);
		    */
		    ...
		    for(int i = 0; i < size / 4; i++)
		    {
		    	...
		    	shrunk_green[i] = (int)green_shrink[i];
		    }
		    ...
		    */
		    ...
		    System.out.println("Number of different values in deltas is " + number_of_different_delta_values);
		    System.out.println("The low value is  " + delta_min);
		    System.out.println("The high value is " + delta_max);
		    
		    
		    int[] inverse_delta = DeltaMapper.getValuesFromDeltas(delta, xdim / 2, ydim / 2);
		    
		    int inverse_delta_min = inverse_delta[0];
		    int inverse_delta_max = inverse_delta[0];
		   
		    for(int i = 0; i < inverse_delta.length; i++)
		    {
		    	if(inverse_delta[i] > inverse_delta_max)
		    		inverse_delta_max = inverse_delta[i];
		    	if(inverse_delta[i] < inverse_delta_min)
		    		inverse_delta_min = inverse_delta[i];
		    }
		    int number_of_different_pixel_values = inverse_delta_max - inverse_delta_min + 1;
		    System.out.println("Range of different pixel values is " + number_of_different_pixel_values );
		    System.out.println("The low value after delta encoding is  " + inverse_delta_min);
		    System.out.println("The high value after delta encoding is " + inverse_delta_max);
		    ...
		    if(number_of_different_delta_values % 2 == 1)
	            n = number_of_different_delta_values / 2;
	        else
	        	n = number_of_different_delta_values / 2 - 1;
	        ...
	        symmetric_lut[n++] = m;
	        m += 2;
	        while(n < number_of_different_delta_values)
	        {
	        	symmetric_lut[n++] = m;
	        	m += 2;
	        }
	        ...
	        */
		    ...
		    if(number_of_different_delta_values % 2 == 1)
		    {
		    	if(lower_value > 0)
		    		...
		    		modal_lut[number_of_different_delta_values - 1] = index;
		    }
		    ...
		    */
		    ...
		    //int number_of_bits  = DeltaMapper.packStrings(delta, size / 4, number_of_different_values, bit_strings, modal_lut);
		    ...
		    //int number_of_ints = DeltaMapper.unpackStrings(bit_strings, number_of_different_values, delta, size / 4, modal_lut);
		    ...
		    for(int i = 0; i < size / 4; i++)
		    {
		    	...
		    	if(value[i] > green_max)
		    		green_max = value[i];
		        int error    = Math.abs(green[i] - value[i]);
		        total_error  += error;
		        biased_error += green[i] - value[i];
		    }
		    System.out.println("The low value is  " + green_min);
		    System.out.println("The high value is " + green_max);
		    System.out.println("Total error is    " + total_error);
		    System.out.println("Biased error is   " + biased_error);
