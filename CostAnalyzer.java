...
public class CostAnalyzer
{
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			System.out.println("Usage: java CostAnalyzer <filename>");
			System.exit(0);
		}
		...
		CostAnalyzer encoder = new CostAnalyzer(prefix + filename);
	}

	public CostAnalyzer(String _filename)
	{
		filename = _filename;
		try
		{
			...
			else	
			{
			    ...
			    JFrame frame = new JFrame("CostAnalyzer");
	...
	class ApplyHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			for(int i = 0; i < 3; i++)
			{
			    ...
			    if(zero_ratio > .5)
				{
					channel_compressed_length[j] = DeltaMapper.compressZeroStrings2(string, channel_length[j], compressed_string);
					channel_bit_type[j] = 0;
				}
			    else
			    {
			    	channel_compressed_length[j] = DeltaMapper.compressOneStrings2(string, channel_length[j], compressed_string);
