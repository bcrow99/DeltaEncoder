		...
		try
		{
		    ...
		    for(int i = 0; i < 3; i++)
		    {
				...
				else if(delta_type == 7)
				{
					...
					/*
				    int seed_map_length =  in.readInt();
				    int zipped_length = in.readInt();
				    byte [] zipped_seed_map = new byte[zipped_length];
				    in.read(zipped_seed_map, 0, zipped_length);
				    byte[] seed_map = new byte[seed_map_length];
				    
				    Inflater inflater = new Inflater();
			        inflater.setInput(zipped_seed_map, 0, zipped_length);
			        System.out.println("Got here.");
			        int unzipped_length = inflater.inflate(seed_map);
			        
			        channel_map_list.add(seed_map);
			        
			        System.out.println("Read and unzipped seed map.");
			        */
