...
public class SimpleReader
{
	...
	ArrayList string_list=new ArrayList(), table_list=new ArrayList();
	ArrayList map_list=new ArrayList(), delta_list=new ArrayList();
	...
	int[][] channel_array=new int[3][0], resize_array=new int[3][0];
	int[]  min=new int[3], init=new int[3], delta_min=new int[3];
	int[]  length=new int[3], compressed_length=new int[3];
	byte[] channel_iterations=new byte[3];

	BufferedImage decoded_image=null, display_image=null;
	ImageCanvas   image_canvas=null;
	JScrollPane   scroll_pane=null;
	JFrame        frame=null;

	double zoom_scale=1.0, fit_scale=1.0;
	...
	public SimpleReader(String filename)
	{
		try {
			...
			xdim=in.readShort(); ydim=in.readShort();
			pixel_shift=in.readByte(); pixel_quant=in.readByte();
			set_id=in.readByte(); delta_type=in.readByte(); entropy_type=in.readByte();

			System.out.println("Image: "+xdim+" x "+ydim);
			System.out.println("Set id: "+set_id+"  Delta type: "+delta_type);

			long start=System.nanoTime();
			for(int i=0;i<3;i++){
				System.out.println("Reading channel "+i);
				min[i]=in.readInt(); init[i]=in.readInt(); delta_min[i]=in.readInt();
				length[i]=in.readInt(); compressed_length[i]=in.readInt(); channel_iterations[i]=in.readByte();

				if(delta_type>=6){
					int map0_adj=in.readInt(); // first map value (original - dmin)
					int ml=in.readInt(); int[] tbl=readTable(in);
					int dmin=in.readInt(), bl=in.readInt();
					byte[] str=new byte[StringMapper.getBytelength(bl)]; in.readFully(str);
					...
					vals[0]=map0_adj; // restore correct first value
					for(int k=0;k<ml;k++) map[k]=(byte)(vals[k]+dmin);
					map_list.add(map);
				}

				// Always string decode
				table_list.add(readTable(in));

				if(entropy_type==0)
				{
					// LZ77 — writer writes payload.length then zl then compressed bytes
					int orig_len=in.readInt();
					byte[] str=new byte[orig_len];
					int zl=in.readInt(); byte[] zd=new byte[zl]; in.readFully(zd);
					Inflater inf=new Inflater(); inf.setInput(zd,0,zl); inf.inflate(str); inf.end();
					string_list.add(str);
				}
				else if(entropy_type==1)
				{
					// Huffman
					int[] rt=readTable(in); int pmin=in.readInt();
					int n=in.readInt(); byte iv=in.readByte(),mx=in.readByte();
					int pdtl=in.readByte()&0xFF; byte[] pdt=new byte[pdtl]; in.readFully(pdt);
					byte[] cl=CodeMapper.unpackLengthTable(n,iv,mx,pdt);
					int hbl=in.readInt(), plen=in.readInt();
					byte[] pb=new byte[plen]; in.readFully(pb);
					huff_rank_list.add(rt); huff_cl_list.add(cl); huff_pay_list.add(pb);
					huff_bl[i]=hbl; huff_pay_min[i]=pmin;
				}
				else
				{
					// Arithmetic (fast or slow)
					int orig_len=in.readInt();
					...
					// store orig_len so Decompressor knows exact payload size
			...
			Thread[] dt=new Thread[3];
			for(int i=0;i<3;i++){dt[i]=new Thread(new Decompressor(i));dt[i].start();}
			for(int i=0;i<3;i++) dt[i].join();
			...
			if(pixel_quant==0){
				...
				image.setRGB(0,0,xdim,ydim,DeltaMapper.getPixel(ResizeMapper.resize(ch[0],intermediate_xdim,xdim,ydim),ResizeMapper.resize(ch[1],intermediate_xdim,xdim,ydim),ResizeMapper.resize(ch[2],intermediate_xdim,xdim,ydim),xdim,pixel_shift),0,xdim);
			}
			System.out.println("RGB assembled in "+((System.nanoTime()-start)/1_000_000)+" ms.");
			decoded_image=image; SwingUtilities.invokeLater(()->showImage());
	...
	private void buildViewer(String filename)
	{
		...
		fit_scale=Math.min(1.0,Math.min((double)(sw*70/100-40)/xdim,(double)(sh*70/100-80)/ydim)); zoom_scale=fit_scale;
		...
		scroll_pane.addMouseWheelListener(e->{if(e.isControlDown()){JViewport vp=scroll_pane.getViewport();Point vp_pos=vp.getViewPosition();Point mp=e.getPoint();int mcx=mp.x+vp_pos.x,mcy=mp.y+vp_pos.y;double old=zoom_scale;zoom_scale=(e.getWheelRotation()<0)?Math.min(ZOOM_MAX,zoom_scale*ZOOM_FACTOR):Math.max(ZOOM_MIN,zoom_scale/ZOOM_FACTOR);if(zoom_scale==old)return;updateDisplayImage();image_canvas.setPreferredSize(new Dimension((int)(xdim*zoom_scale),(int)(ydim*zoom_scale)));image_canvas.revalidate();image_canvas.repaint();double r=zoom_scale/old;vp.setViewPosition(new Point(Math.max(0,(int)(mcx*r)-mp.x),Math.max(0,(int)(mcy*r)-mp.y)));updateTitle();}else scroll_pane.dispatchEvent(e);});
		frame=new JFrame("Simple Reader [decoding...]");
		frame.addWindowListener(new WindowAdapter(){public void windowClosing(WindowEvent e){System.exit(0);}});
		JMenuBar mb=new JMenuBar(); JMenu vm=new JMenu("View");
		...
		mb.add(vm); frame.setJMenuBar(mb);
		...
		frame.setSize(display_w + overhead_w, display_h + overhead_h);
		frame.setLocation(5,5); frame.setVisible(true);
	}

	private void showImage(){Dimension v=scroll_pane.getViewport().getSize();zoom_scale=Math.min(1.0,Math.min(v.width>0?(double)v.width/xdim:1.0,v.height>0?(double)v.height/ydim:1.0));updateDisplayImage();image_canvas.setPreferredSize(new Dimension((int)(xdim*zoom_scale),(int)(ydim*zoom_scale)));image_canvas.revalidate();image_canvas.repaint();updateTitle();}
	private void zoomBy(double f){double ns=Math.max(ZOOM_MIN,Math.min(ZOOM_MAX,zoom_scale*f));if(ns==zoom_scale)return;JViewport vp=scroll_pane.getViewport();Point p=vp.getViewPosition();Dimension vs=vp.getSize();double cx=p.x+vs.width/2.0,cy=p.y+vs.height/2.0,r=ns/zoom_scale;zoom_scale=ns;updateDisplayImage();image_canvas.setPreferredSize(new Dimension((int)(xdim*zoom_scale),(int)(ydim*zoom_scale)));image_canvas.revalidate();image_canvas.repaint();vp.setViewPosition(new Point(Math.max(0,(int)(cx*r-vs.width/2.0)),Math.max(0,(int)(cy*r-vs.height/2.0))));updateTitle();}
	private void updateDisplayImage(){if(decoded_image==null)return;if(zoom_scale==1.0){display_image=decoded_image;return;}int w=Math.max(1,(int)(xdim*zoom_scale)),h=Math.max(1,(int)(ydim*zoom_scale));AffineTransform t=new AffineTransform();t.scale(zoom_scale,zoom_scale);display_image=new AffineTransformOp(t,AffineTransformOp.TYPE_BILINEAR).filter(decoded_image,new BufferedImage(w,h,decoded_image.getType()));}
	private void updateTitle(){if(frame!=null)frame.setTitle("Simple Reader ["+(decoded_image==null?"decoding...":(int)Math.round(zoom_scale*100)+"%")+"]");}

	private static int[] readTable(DataInputStream in) throws IOException
	{
		int tl=in.readShort(); int[] tbl=new int[tl]; int max=Byte.MAX_VALUE*2+1;
		if(tl<=max) for(int k=0;k<tl;k++){tbl[k]=in.readByte();if(tbl[k]<0)tbl[k]=max+1+tbl[k];}
		else for(int k=0;k<tl;k++) tbl[k]=in.readShort();
		return tbl;
	}

	class ImageCanvas extends JPanel{public ImageCanvas(){setOpaque(true);}@Override public Dimension getPreferredSize(){return display_image!=null?new Dimension(display_image.getWidth(),display_image.getHeight()):new Dimension(Math.max(1,(int)(xdim*zoom_scale)),Math.max(1,(int)(ydim*zoom_scale)));}@Override protected synchronized void paintComponent(Graphics g){super.paintComponent(g);if(display_image!=null)g.drawImage(display_image,0,0,this);}}
	class Resizer implements Runnable{int[]src;int xdim,nx,ny,i;Resizer(int[]s,int x,int nx,int ny,int i){src=s;this.xdim=x;this.nx=nx;this.ny=ny;this.i=i;}public void run(){resize_array[i]=ResizeMapper.resize(src,xdim,nx,ny);}}
	...
	class Decompressor implements Runnable
	{
		...
		public void run()
		{
			try {
				...
				if(entropy_type==0)
				{
					// LZ77 already inflated at read time
					payload=(byte[])string_list.get(i);
				}
				else if(entropy_type==1)
				{
					// Huffman
					int[] rt=huff_rank_list.get(i); byte[] cl=huff_cl_list.get(i);
					byte[] pb=huff_pay_list.get(i); int hbl=huff_bl[i]; int pmin=huff_pay_min[i];
					int[] hc=CodeMapper.getCanonicalCode(cl);
					int sl=compressed_length[i]/8; if(compressed_length[i]%8!=0)sl++; sl++;
					int[] dec=new int[sl]; CodeMapper.unpackCode(pb,rt,hc,cl,hbl,dec);
					payload=new byte[sl];
					for(int k=0;k<sl;k++) payload[k]=(byte)(dec[k]+pmin);
				}
				else
				{
					// Arithmetic — recover exact payload length stored in string_list
				...
				// String decode: bit-decompress then unpack to delta values
