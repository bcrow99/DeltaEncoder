...
public class DeltaEncoder
{
	...
	public DeltaEncoder(String _filename)
	{
		...
		try
		{
			...
			zipped_pixel_strings = new byte[xdim * ydim * 25];
			zipped_delta_strings = new byte[xdim * ydim * 25];
	...
	class SaveHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent event)
		{
			...
			String zip_compressed_pixel_filename = new String(root + "_cp_" + pixel_shift + ".gz");
			String zip_compressed_delta_filename = new String(root + "_cd_" + pixel_shift + ".gz");
			...
			else
			{
			    ...
			    try
			    {
			        ...
			        GZIPOutputStream compressed_pixel_zip = new GZIPOutputStream(new FileOutputStream(zip_compressed_pixel_filename)); 
			        int number_of_compressed_bytes = compressed_pixel_length / 8;
			        if(compressed_pixel_length  % 8 != 0)
			        	number_of_compressed_bytes++;
			        compressed_pixel_zip.write(compressed_pixel_strings, 0, number_of_compressed_bytes);
			        compressed_pixel_zip.close();
			        output_file = new File(zip_compressed_pixel_filename);   
			        length = output_file.length();
			        System.out.println("Length of file is " + length);
			        ratio = (double)length / (xdim * ydim);
			        System.out.println("The ratio of zipped compressed_packed pixel bits in a file to pixel bits is " + String.format("%.2f", ratio));
			        
			        
			        
			        number_of_compressed_bytes = compressed_delta_length / 8;
			        if(compressed_delta_length  % 8 != 0)
			        	number_of_compressed_bytes++;
			        
			        System.out.println("The number of compressed bytes is " + number_of_compressed_bytes);
			        GZIPOutputStream compressed_delta_zip = new GZIPOutputStream(new FileOutputStream(zip_compressed_delta_filename)); 
			        compressed_delta_zip.write(compressed_delta_strings, 0, number_of_compressed_bytes);
			        compressed_delta_zip.close();
			        output_file = new File(zip_delta_filename);   
			        length = output_file.length();
			        System.out.println("Length of file is " + length);
			        ratio = (double)length / (xdim * ydim);
			        System.out.println("The ratio of zipped compressed packed delta bits in a file to pixel bits is " + String.format("%.2f", ratio));
