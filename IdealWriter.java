...
public class IdealWriter
{
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			System.out.println("Usage: java IdealWriter <filename>");
			System.exit(0);
		}
		String prefix       = new String("C:/Users/Brian Crowley/Desktop/");
		String filename     = new String(args[0]);
	
		IdealWriter writer = new IdealWriter(prefix + filename);
	}

	public IdealWriter(String _filename)
	{
		filename = _filename;
		try
		{
	...
	class ApplyHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
		    ...
		    for(int i = 0; i < 6; i++)
		    {
		    	...
		    	channel_sum[i] = DeltaMapper.getIdealSum2(quantized_channel, new_xdim, new_ydim, 20);
		    }
			...
			for(int i = 0; i < 3; i++)
			{
				...
				{
					result = DeltaMapper.getDeltasFromValues3(quantized_channel, new_xdim, new_ydim);
			...
			for(int i = 0; i < 3; i++)
		    {
			    ...
			    if(use_map)
			    {
			    	...
			    	channel = DeltaMapper.getValuesFromDeltas3(delta, new_xdim , new_ydim, channel_init[j]);
				if(j > 2)
