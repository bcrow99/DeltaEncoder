...
public class GetSums
{
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			System.out.println("Usage: java GetSums <filename>");
			System.exit(0);
		}
		...
		GetSums encoder = new GetSums(prefix + filename);
	}

	public GetSums(String _filename)
	{
		filename = _filename;
		try
		{
	...
	class ApplyHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent event)
		{
			...
			int sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the green sub deltas is " + sum);
	    	...
	    	//System.out.println("The ratio of zipped green sub delta bytes to pixel bits is " + String.format("%.4f", ratio));
			...
			ratio = delta_length;
			ratio /= pixel_length;
			//System.out.println("The ratio of green sub delta string bits to pixel bits is " + String.format("%.4f", ratio));
	    	...
	    	ratio /= pixel_length;
	    	//System.out.println("The ratio of zipped sub delta string bits to pixel bits is " + String.format("%.4f", ratio));
			...
			if(compress)
			{
				...
				ratio /= pixel_length;
				//System.out.println("The ratio of compressed sub delta string bits to pixel bits is " + String.format("%.4f", ratio));	
			    ...
			    ratio = zipped_length * 8;
			    ratio /= pixel_length;
			    //System.out.println("The ratio of zipped compressed sub delta string bits to pixel bits is " + String.format("%.4f", ratio));
			}
			//System.out.println();
	          
		   
			init_value       = shifted_green[0];
			delta            = DeltaMapper.getDeltasFromValues2(shifted_green, xdim, ydim, init_value);
			...
			sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the green up  deltas is " + sum);
			...
			ratio = delta_length;
			ratio /= pixel_length;
			//System.out.println("The ratio of green up delta string bits to pixel bits is " + String.format("%.4f", ratio));
	    	...
	    	ratio = zipped_length * 8;
	    	ratio /= pixel_length;
	    	//System.out.println("The ratio of zipped up delta string bits to pixel bits is " + String.format("%.4f", ratio));
			...
			if(compress)
			{
				...
				ratio  = compressed_delta_length;
				ratio /= pixel_length;
				//System.out.println("The ratio of compressed up delta string bits to pixel bits is " + String.format("%.4f", ratio));	
			...
			sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the  blue sub deltas is " + sum);
			
			delta            = DeltaMapper.getDeltasFromValues2(shifted_blue, xdim, ydim, init_value);
			histogram_list   = DeltaMapper.getHistogram(delta);
			delta_min        = (int)histogram_list.get(0);
			histogram        = (int[])histogram_list.get(1);
			delta_random_lut = DeltaMapper.getRandomTable(histogram); 
			for(int i = 0; i < delta.length; i++)
			    delta[i] -= delta_min;
			sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the  blue up  deltas is " + sum);
			...
			for(int i = 0; i < delta.length; i++)
				delta[i] -= delta_min;
			sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the   red sub deltas is " + sum);
			
			delta            = DeltaMapper.getDeltasFromValues2(shifted_red, xdim, ydim, init_value);
			histogram_list   = DeltaMapper.getHistogram(delta);
		    delta_min        = (int)histogram_list.get(0);
			histogram        = (int[])histogram_list.get(1);
			delta_random_lut = DeltaMapper.getRandomTable(histogram); 
			for(int i = 0; i < delta.length; i++)
				delta[i] -= delta_min;
			sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the   red up  deltas is " + sum);
		    ...
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the blue green sub deltas is " + sum);
		    
		    delta            = DeltaMapper.getDeltasFromValues2(shifted_blue_green, xdim, ydim, init_value);
		    histogram_list   = DeltaMapper.getHistogram(delta);
		    delta_min        = (int)histogram_list.get(0);
		    histogram        = (int[])histogram_list.get(1);
		    delta_random_lut = DeltaMapper.getRandomTable(histogram); 
		    for(int i = 0; i < delta.length; i++)
		    	delta[i] -= delta_min;
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the blue green up  deltas is " + sum);
		    ...
		    for(int i = 0; i < delta.length; i++)
		    	delta[i] -= delta_min;
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the  red green sub deltas is " + sum);
		    
			delta            = DeltaMapper.getDeltasFromValues2(red_green, xdim, ydim, init_value);
		    histogram_list   = DeltaMapper.getHistogram(delta);
		    delta_min        = (int)histogram_list.get(0);
		    histogram        = (int[])histogram_list.get(1);
		    delta_random_lut = DeltaMapper.getRandomTable(histogram); 
		    for(int i = 0; i < delta.length; i++)
		    	delta[i] -= delta_min;
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the  red green up  deltas is " + sum);
		    delta_length = 0;
		    delta_length  = DeltaMapper.packStrings(delta, delta_random_lut, delta_strings);
		    ratio = delta_length;
			ratio /= pixel_length;
		    ...
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the   red blue sub deltas is " + sum);
		    
			delta            = DeltaMapper.getDeltasFromValues2(red_blue, xdim, ydim, init_value);
		    histogram_list   = DeltaMapper.getHistogram(delta);
		    delta_min        = (int)histogram_list.get(0);
		    histogram        = (int[])histogram_list.get(1);
		    delta_random_lut = DeltaMapper.getRandomTable(histogram); 
		    ...
		    sum = DeltaMapper.getSum(delta);
			System.out.println("The sum of the   red blue up  deltas is " + sum);
