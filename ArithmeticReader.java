...
public class ArithmeticReader
{
	...
	ArrayList<byte[][]>                 order_list    = new ArrayList<>();  // null entry when that channel used no order table
	boolean[] channel_use_order = new boolean[3];
	...
	public ArithmeticReader(String filename)
	{
		try
		{
			...
			slow_arithmetic = (in.readByte() != 0);
			data_type   = in.readByte();

			System.out.println("Image: " + xdim + " x " + ydim);
			System.out.println("Set id: " + set_id + "  Delta type: " + delta_type + "  Arithmetic: " + (slow_arithmetic ? "Slow" : "Fast"));
			...
			for (int i = 0; i < 3; i++)
			{
				...
				// Arithmetic only — always String payload
				...
				if (slow_arithmetic)
				{
					...
					for (int m = 0; m < n_segs; m++)
					{
						...
						bb = new byte[ll];
						in.readFully(bb);
						offsets[m][0] = new java.math.BigInteger(bb);
						ll = in.readInt();
						bb = new byte[ll];
						in.readFully(bb);
						offsets[m][1] = new java.math.BigInteger(bb);
					}
			...
			if (set_id == 0)
			{
				ch[0] = channel_array[0];
				ch[1] = channel_array[1];
				ch[2] = channel_array[2];
			}
			else if (set_id == 1)
			{
				ch[0] = channel_array[0];
				ch[1] = DeltaMapper.getDifference(channel_array[1], channel_array[2]);
				ch[2] = channel_array[1];
			}
			else if (set_id == 2)
			{
				ch[0] = channel_array[0];
				ch[1] = DeltaMapper.getDifference(channel_array[0], channel_array[2]);
				ch[2] = channel_array[1];
			}
			else if (set_id == 3)
			{
				ch[0] = channel_array[0];
				ch[1] = DeltaMapper.getDifference(channel_array[0], channel_array[1]);
				ch[2] = DeltaMapper.getSum(channel_array[2], ch[1]);
			}
			else if (set_id == 4)
			{
				ch[0] = channel_array[0];
				ch[1] = DeltaMapper.getDifference(channel_array[0], channel_array[1]);
				ch[2] = DeltaMapper.getSum(channel_array[0], channel_array[2]);
			}
			else if (set_id == 5)
			{
				ch[0] = DeltaMapper.getSum(channel_array[2], channel_array[0]);
				ch[1] = channel_array[0];
				ch[2] = channel_array[1];
			}
			else if (set_id == 6)
			{
				...
				ch[1] = DeltaMapper.getSum(channel_array[2], channel_array[0]);
				ch[0] = DeltaMapper.getSum(channel_array[1], ch[1]);
				ch[2] = channel_array[0];
			}
			else if (set_id == 7)
			{
				ch[0] = DeltaMapper.getSum(channel_array[0], channel_array[1]);
				ch[1] = channel_array[0];
				ch[2] = DeltaMapper.getSum(channel_array[0], channel_array[2]);
			}
			else if (set_id == 8)
			{
				ch[2] = DeltaMapper.getSum(channel_array[0], channel_array[1]);
				ch[0] = DeltaMapper.getDifference(ch[2], channel_array[2]);
				ch[1] = channel_array[0];
			}
			else
			{
				ch[0] = DeltaMapper.getDifference(channel_array[0], channel_array[2]);
				ch[1] = DeltaMapper.getDifference(channel_array[0], channel_array[1]);
				ch[2] = channel_array[0];
			}
			...
			else if (xdim > 600)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					rt[i] = new Thread(new Resizer(ch[i], intermediate_xdim, xdim, ydim, i));
					rt[i].start();
				}
				for (int i = 0; i < 3; i++)
				{
	...
	private void buildViewer(String filename)
	{
		...
		int sw = (int) sc.getWidth();
		int sh = (int) sc.getHeight();
	...
	private void showImage()
	{
		...
		double vw = (v.width > 0) ? (double) v.width / xdim : 1.0;
		double vh = (v.height > 0) ? (double) v.height / ydim : 1.0;
	...
	private void zoomBy(double factor)
	{
		...
		double cx = p.x + vs.width / 2.0;
		double cy = p.y + vs.height / 2.0;
	...
	{
		if (frame != null)
			frame.setTitle("Arithmetic Reader  [" + (decoded_image == null ? "decoding\u2026" : (int) Math.round(zoom_scale * 100) + "%") + "]");
	}
	...
	class Resizer implements Runnable
	{
		...
		Resizer(int[] src, int xdim, int nx, int ny, int i)
		{
			this.src = src;
			this.xdim = xdim;
			this.nx = nx;
		...
		public void run()
		{
			resize_array[i] = ResizeMapper.resize(src, xdim, nx, ny);
		}
	}
		...
		public void run()
		{
			try
			{
				...
				else
				{
					...
					cur_xdim = intermediate_xdim;
					cur_ydim = intermediate_ydim;
				}
				size = cur_xdim * cur_ydim;

				// Entropy decode -> payload bytes (arithmetic only)
				...
				if (slow_arithmetic)
				{
					...
					byte[][]                 orders  = order_list.get(i);
					boolean                  use_order = channel_use_order[i];
					...
					for (int p = 0; p < n_procs; p++)
					{
						...
						at[p] = new Thread(() ->
						{
							...
							{
								if (use_order)
									segs[m] = ArithmeticMapper.getArithmeticValues(offsets[m], freqs[m], segs[m].length, orders[m]);
								else
					...
					System.out.println(String.format("  Slow arithmetic decode ch%d (order=%s): %d segs, %d threads, %d ms",
						i, use_order ? "yes" : "none", n_segs, n_procs, (System.nanoTime() - arith_start) / 1_000_000));
				}
				else
				{
				...
				for (int m = 0; m < n_segs; m++)
				{
					System.arraycopy(segs[m], 0, payload, pos, segs[m].length);
					pos += segs[m].length;
				}
				...
				int[] cur_ch;
				if (delta_type == 0)
					cur_ch = DeltaMapper.getValuesFromHorizontalDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 1)
					cur_ch = DeltaMapper.getValuesFromVerticalDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 2)
					cur_ch = DeltaMapper.getValuesFromAverageDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 3)
					cur_ch = DeltaMapper.getValuesFromMedDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 4)
					cur_ch = DeltaMapper.getValuesFromAdaptiveDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 5)
					cur_ch = DeltaMapper.getValuesFromDirectionalDeltas(delta, cur_xdim, cur_ydim, init[i]);
				else if (delta_type == 6)
					cur_ch = DeltaMapper.getValuesFromMixedDeltas(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));
				else if (delta_type == 7)
					cur_ch = DeltaMapper.getValuesFromMixedDeltas2(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));
				else if (delta_type == 8)
					cur_ch = DeltaMapper.getValuesFromMixedDeltas4(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));
				else if (delta_type == 9)
					cur_ch = DeltaMapper.getValuesFromMixedDeltas16Rows(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));
				else if (delta_type == 10)
					cur_ch = DeltaMapper.getValuesFromIdealDeltas8(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));
				else
					cur_ch = DeltaMapper.getValuesFromIdealDeltas16(delta, cur_xdim, cur_ydim, init[i], (byte[]) map_list.get(i));

				if (channel_id[i] > 2)
					for (int j = 0; j < cur_ch.length; j++)
					{
						cur_ch[j] += min[i];
					}
			...
			{
				System.out.println("Decompressor " + i + ": " + e);
			}
		}
	}
}