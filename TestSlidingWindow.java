...
public class TestSlidingWindow
{
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			System.out.println("Usage: java TestSlidingWindow <filename>");
			System.exit(0);
		}
		...
		TestSlidingWindow encoder = new TestSlidingWindow(prefix + filename);
	}

	public TestSlidingWindow(String _filename)
	{
		filename = _filename;
		try
		{
			...
			if(raster_type == BufferedImage.TYPE_3BYTE_BGR)
			{
			    ...
			    JFrame frame = new JFrame("TestSlidingWindow");
	...
	class ApplyHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent event)
		{
		    ...
		    for(int i = 0; i < 6; i++)
		    {
				...
				int sliding_window_length = window_length * (length3 / 2 - 64) / 100 + 64;
				
				
				ArrayList ratio_list = DeltaMapper.getSlidingWindowRatio(delta_string, length3, sliding_window_length);
				double [] zero_one_ratio = (double [])ratio_list.get(0);
				double max_ratio = 0;
				double min_ratio = 1;
				
				for(int j = 0; j < zero_one_ratio.length; j++)
				{
				    if(max_ratio < zero_one_ratio[j])
				    	max_ratio = zero_one_ratio[j];
				    if(min_ratio > zero_one_ratio[j])
				    	min_ratio = zero_one_ratio[j];
				}
				
				System.out.println(channel_string[i] + ":");
				System.out.println("The minimum ratio in a window is " + String.format("%.2f", min_ratio));
				System.out.println("The maximum ratio in a window is " +  String.format("%.2f", max_ratio));
				System.out.println();
		    }  
