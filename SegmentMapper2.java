		...
		*/
		...
		for(int i = 0; i < number_of_segments; i++)
		{
			...
			if(type == 0)
			{
				...
				{
					System.out.println("Length of original segment is " + segment.length);
					System.out.println("Length of decompressed segment is " + decompressed_segment.length);
				}
				else
				{
					boolean same = true;
		    	    ...
		    	    while(same && current_index < segment.length)
		    	    {
		    	        if(decompressed_segment[current_index] != segment[current_index])	
		    	        {
                            ...
                            System.out.println("The length of the segment is " + segment.length);
		    	            System.out.println();
				...
				if(i < current_number_of_segments - 1)
				{
				    ...
				    if(current_iterations == next_iterations)
				    {
				        ...
				        else
				        {
				    	    ...
				    	    if(merged_bitlength  < decompressed_combined_bitlength)
				    	    {
				    	    	merged_list.add(merged_segment);
					    	    ...
					    	    /*
					    	    byte [] uncompressed_segment = StringMapper.decompressStrings(merged_segment);
					    	    
					    	    boolean same = true;
					    	    int current_index = 0;
					    	    while(same && current_index < uncompressed_merged_segment.length)
					    	    {
					    	        if(uncompressed_merged_segment[current_index] != uncompressed_segment[current_index])	
					    	        {
					    	        	int merged_iterations = StringMapper.getIterations(merged_segment);
					    	        	int merged_bitlength2  = StringMapper.getBitlength(merged_segment);
					    	        	System.out.println("Merged iterations was " + merged_iterations);
					    	        	System.out.println("Merged bit length was " + merged_bitlength2);
					    	        	
					    	        	//int uncompressed_bit_length2 = StringMapper.getBitlength(uncompressed_segment);
					    	        	int iterations2 = StringMapper.getIterations(uncompressed_segment);
					    	        	System.out.println("Decompressed iterations was " + iterations2);
					    	        	System.out.println();
