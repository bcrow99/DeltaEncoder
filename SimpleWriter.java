...
public class SimpleWriter
{
	...
	public static void main(String[] args)
	{
		...
		// init() is now called from showInitialImage() after the UI is fully built
	}
	...
	public SimpleWriter(String _filename)
	{
		...
		try
		{
			...
			if (raster_type == BufferedImage.TYPE_3BYTE_BGR)
			{
				...
				// Temporary default selection — showInitialImage() will correct this
				// after init() has determined the best delta_type.
	...
	private void showInitialImage()
	{
		...
		// ButtonGroup ensures the previously selected button is deselected automatically.
		init();
		delta_button[delta_type].setSelected(true);
	}
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				if (delta_type == 0)
					...
					channel = DeltaMapper.getValuesFromMixedDeltas(delta, new_xdim, new_ydim, channel_init[j], (byte[]) map_list.get(i));
				else if (delta_type == 6)
					channel = DeltaMapper.getValuesFromMixedDeltas2(delta, new_xdim, new_ydim, channel_init[j], (byte[]) map_list.get(i));
				else if (delta_type == 7)
					channel = DeltaMapper.getValuesFromIdealDeltas(delta, new_xdim, new_ydim, channel_init[j], (byte[]) map_list.get(i));
